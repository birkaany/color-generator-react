...
export default function PaletteListItem({ savedPaletteColor, setSavedColor, id }) {
  ...
  return (
    <li className="group flex relative">
      <div className="opacity-0 absolute flex justify-center items-center gap-10 w-full h-full top-0 left-0 z-20 bg-gray-300 transition-opacity duration-300 group-hover:opacity-100 ">
      ...
      {savedPaletteColor.map((color, index) => {
        return <div key={index} style={{ background: `rgb(${color})` }} className="w-full h-10"></div>;
      })}
    </li>
  );
}
